...
unanswerable_keyphrases = [
    ...
    "No information provided in this article.",
...
def compute_metrics(prediction: str,
                    sub_ref: List[str],
                    org_ref: List[str],
                    dataset_name) -> Dict[str,
                                          float]:
    scores = {}
    if dataset_name != "memo-trap":
        ...
        model_ans = normalize_answer(prediction)
        ground_truth = normalize_answer(sub_ref[0])

        scores["sub_EM"] = float(model_ans.lower() == ground_truth.lower())
        scores["sub_Subspan_EM"] = float(sub_ref[0] in prediction)
        if sub_ref[0] in [" A", " B"]:
            scores["sub_Subspan_EM"] = float(sub_ref[0] in prediction) or float(sub_ref[0].strip()+"." in prediction)
        scores["org_EM"] = best_em(prediction, org_ref)
        scores["org_Subspan_EM"] = best_subspan_em(prediction, org_ref)
...
def em_and_subem(predictions, dataset_name):
    if dataset_name == "eli5":
        rouge = Rouge()
        hypotheses = [prediction['predicted_answer']
                      for prediction in predictions]
        references = [prediction['sub_answer'][0]
                      for prediction in predictions]
    ...
    for sample in predictions:
        ...
        scores = compute_metrics(prediction, sub_ref, org_ref, dataset_name)
...
def get_zero_subspan_em_indices(predictions, dataset_name):
    zero_subspan_em_indices = []
    ...
    for idx, sample in enumerate(predictions):
        ...
        if dataset_name == "memo-trap":
            ...
            rouge = Rouge()
            scores = rouge.get_scores(prediction, sub_ref[0])
            if scores[0]['rouge-l']['r'] <= 0.05:
                zero_subspan_em_indices.append(idx)
        elif best_subspan_em(prediction, sub_ref) == 0.0:
            zero_subspan_em_indices.append(idx)
    return zero_subspan_em_indices
...
#     return  factkb_tokenizer, factkb_model


def get_sub_answers(eval_dataset, ground_truth_file):
    sub_answers = []
    if eval_dataset == "nqopen":
        with open(ground_truth_file, 'r') as f:
            for line in f:
                sub_answers.append(json.loads(line)['answers'])
    elif eval_dataset == "triviaqa":
        with open(ground_truth_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
            # 提取Data数组中每个元素的Answer.Value
            for item in data:  # ← 新增代码开始
                answer_value = item.get('Answer', {}).get('Value')
                sub_answers.append([answer_value])
    elif eval_dataset == "memo-trap":
        data_path = 'data/memo_trap'
        for file_name in [
        ...
        with open(ground_truth_file, 'r', encoding='utf-8') as f:
            ...
            for item in data:
                ...
                sub_answers.append([answer_value])  # ELI5只需要字符串，不需要列表
    return sub_answers
