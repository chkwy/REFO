...
Functions:
    get_prompt_list(args):
        ...
        Returns:
            ...
            question_list: A list of original questions or prompts.
    main():
...
def get_prompt_list(args):
    ...
    question_list = []  # 初始化 question_list
    ...
    if args.eval_dataset == "nqswap":
        ...
        for data in data_list:
            ...
            question_prompt = f"Based on the following text:\n{passage}\nAnswer the following question: {question}\nPlease directly response the accurate object"
            question_list.append(question_prompt)  # 添加完整的question_prompt到question_list
            question_formatted = [
                {"role": "system", "content": ""},
        ...
        with open(input_datapath, 'r') as f:
            ...
            for data in data_list[:50]:
                ...
                question_list.append(f"Article: {data['document']}\n\nSummary:")
    elif args.eval_dataset == "memo-trap":
        prompt_list = []
        for file_name in [
            ...
            for prompt in data_list:
                question_list.append(prompt+"\nRemember to end with the period '.' ")  # 添加原始prompt到question_list
                question = [
                    {"role": "system", "content": ""},
                    {"role": "user", "content": prompt+"\nRemember to end with the period '.' ", }
                ]
                prompt_list.append(
                    tokenizer.apply_chat_template(
                        question,
        ...
        for data in data_list:
            ...
            question_list.append(question[1]["content"])
            prompt_list.append(
                tokenizer.apply_chat_template(
                    question,
                    tokenize=False,
        ...
        data_list = load_data(input_datapath)
        print("number of samples in the dataset:", len(data_list))
        prompt_list,question_list = get_inputs(
            data_list,
            args.eval_dataset,
            tokenizer,
    ...
    return prompt_list, input_datapath, question_list
...
def main():
    ...
    prompt_list, ground_truth_file, question_list = get_prompt_list(args)
    ...
    for i in range(1):  # set to 3 for 3 times evaluation
        # part 2: generation
        outputs = model_vllm.generate(prompt_list, sampling_params,lora_request=LoRARequest("adapter", 1, model_path))#generation
        output_list = []
        ...
        for prompt, output, ground_truth in zip(
            ...
            result.append({"prompt": prompt, "output": output,
                          "ground_truth": ground_truth, "question": question_list[i]})
        ...
        # Evaluation
            ...
            if args.eval_dataset == "memo-trap":
                data_path_memotrap = 'data/memo_trap'
                index=0
                for file_name in [
                    "1-proverb-ending.csv",
                    "2-proverb-translation.csv",
                    ...
                    "4-history-of-science-qa.csv"]:
                    # for file_name in ["4-history-of-science-qa.csv"]:
                    file_path = os.path.join(data_path_memotrap, file_name)
                    data_sub_file = pd.read_csv(file_path)
                    len_sub_file= len(data_sub_file)
                    data_sub = data[index:index+len_sub_file]
                    index+=len_sub_file
                    data_sub_spanem=em_and_subem(data_sub, args.eval_dataset)
                    metric[file_name] = data_sub_spanem["sub_Subspan_EM"]
    ...
    if args.eval_dataset == 'eli5':
        ...
        final_metric["sub_Subspan_EM"] = final_metric["rouge-l"]["f"]
    else:
