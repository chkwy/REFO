...
def filter_high_quality_samples(
    ...
    start_idx = int(total_samples * percentage)
    end_idx = int(total_samples * (percentage + 0.64))
    
    # 确保索引在有效范围内
    start_idx = max(0, min(start_idx, total_samples - 1))
    end_idx = max(0, min(end_idx, total_samples))
    
    # 获取指定范围内的样本并提取original_data
    high_quality_samples = [item.get('original_data', {}) for item in input_data[start_idx:end_idx] if item.get('score_distance', 0) > 0.2]
