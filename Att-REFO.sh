...
initial_model_name=${3:-Llama_3_8B_Residual_with_attention_score_1}   
...
#Llama
filter_threshold_list=(0.2 0.6 0.4 0.4 0.4 0.4 0.4 0.4 0.4 0.4)
residual_threshold_list=(0.2 0.3 0.3 0.3 0.4 0.4 0.4 0.4 0.4 0.4)
...
compute_attention_score(){
    ...
    if [ ! -f "$output_path" ]; then
        python SEFO/compute_attention_score_lora.py \
...
ratios = [item['ratio'] for item in data if 'ratio' in item]

avg_ratio = np.mean(ratios)
print(f'{avg_ratio:.6f}')
with open('/tmp/ratio_stats.txt', 'w') as fw:
    print(f'Ratio平均值: {avg_ratio:.6f}', file=fw)
    print(f'总样本数: {len(ratios)}', file=fw)
    print(f'Ratio范围: {min(ratios):.6f} - {max(ratios):.6f}', file=fw)
" > /tmp/avg_ratio.txt

    avg_ratio=$(cat /tmp/avg_ratio.txt)
    
    export ATTENTION_SCORE_AVG_RATIO="$avg_ratio"
    export ATTENTION_SCORE_FILE="$output_path"
    export ATTENTION_INDEX=0
    export LAMBDA_WEIGHT="$lambda_weight"
    echo "已计算注意力文件: $output_path" >> "$run_log_file"
    echo "Ratio平均值: $avg_ratio" >> "$run_log_file"
    cat /tmp/ratio_stats.txt >> "$run_log_file"
    rm -f /tmp/avg_ratio.txt /tmp/ratio_stats.txt
}
...
check_early_stopping() {
    ...
    local global_best_score="$2"
    local improvement_threshold="${3:-0.001}"  # 默认改进阈值为0.001
    
    # 计算改进幅度
    improvement_check=$(python3 -c "
current = float('$current_score')
best = float('$global_best_score')
improvement = current - best
print(f'{improvement:.6f}')
print('continue' if improvement > $improvement_threshold else 'stop')
")
    
    local improvement=$(echo "$improvement_check" | head -n1)
    local should_continue=$(echo "$improvement_check" | tail -n1)
    
    # 返回结果：should_continue 和 improvement
    echo "$should_continue"
    echo "$improvement"
}

# 更新全局最佳分数函数
update_global_best_score() {
    local current_score="$1"
    local global_best_score="$2"
    
    cmp=$(python3 -c "print('1' if float('$current_score') > float('$global_best_score') else '0')")
    if [ "$cmp" = "1" ]; then
        echo "$current_score"
    else
        echo "$global_best_score"
    fi
}
...
get_model_score() {
    ...
    for dataset in "${datasets[@]}"; do
        local metrics_file="eval/outputs/${dataset}_${model_name}_metrics_final.json"
        if [ -f "$metrics_file" ]; then
            local score=$(cat "$metrics_file" 2>/dev/null | \
                tr ',' '\n' | grep '"sub_Subspan_EM"' | cut -d':' -f2 | tr -d ' }' | \
                awk '{printf "%.4f", $1}' 2>/dev/null || echo "0.0")
            total_score=$(python3 -c "print($total_score + float('$score'))")
            valid_datasets=$((valid_datasets + 1))
        fi
    done
    
    # 计算平均分数
    if [ $valid_datasets -gt 2 ]; then
        local avg_score=$(python3 -c "print('%.4f' % ($total_score / $valid_datasets))")
        echo "$avg_score"
    else
        echo "0.0"
...
for i in $(seq 1 $num_iterations); do
    ...
    compute_attention_score "data/${filtered_dataset_name_step1}.json" "data/${filtered_dataset_name_step1}_with_attention_score.json" "$model_path_step0" "$base_model_path" >> "$run_log_file" 2>&1

    # 第2步：训练模型
    echo "步骤2: 训练模型" >> "$run_log_file"
    model_name_step1="${initial_model_name}_iter${i}_step1"
    export NAME="$model_name_step1"
    ...
    # 获取当前模型分数
    model_step1_score=$(get_model_score "$model_name_step1")
    # 如果best score变了就更改model path prev
    if [ $(echo "$model_step1_score > $global_best_score" | bc -l) -eq 1 ]; then
        model_path_prev="/data/DFO_wang/saves/${model_name_step1}"
    fi
    # 更新global_best_score
    global_best_score=$(update_global_best_score "$model_step1_score" "$global_best_score")
    ...
    for step in "${step_list[@]:1}"; do  # 从步骤2开始
        ...
        compute_attention_score "data/${filtered_dataset_name_step}.json" "data/${filtered_dataset_name_step}_with_attention_score.json" "$model_path_prev" "$base_model_path" >> "$run_log_file" 2>&1
        
        # 训练模型
        echo "步骤$((step*2+1)): 训练模型" >> "$run_log_file"
        model_name_step="${initial_model_name}_iter${i}_step${step}"
        export NAME="$model_name_step"
        ...
        # 获取当前模型分数
        model_to_train_score=$(get_model_score "$model_name_step")

        # 早停判断
        if [ $step -gt 1 ]; then
            early_stop_result=$(check_early_stopping "$model_to_train_score" "$global_best_score")
            should_continue=$(echo "$early_stop_result" | head -n1)
            improvement=$(echo "$early_stop_result" | tail -n1)
        ...
        # 更新global_best_score
        global_best_score=$(update_global_best_score "$model_to_train_score" "$global_best_score")
